...
Commands must:
- Load config via `RepoManifest.load()` — never hardcode prefixes or paths
...
Releases are tagged on `main` following semver. Bump both version fields before tagging:

```bash
# 1. Update versions
#    cli/pyproject.toml → version = "0.2.0"
#    package.json       → "version": "0.2.0"

git add cli/pyproject.toml package.json
git commit -m "chore: PLAT-N Bump to v0.2.0"
git tag v0.2.0
git push origin main v0.2.0
```

Teams consuming the platform pin to a tag. Upgrade is a one-liner:
...
# or
pnpm add github:JulioElCesar/golden-path#v0.2.0
```

Breaking changes require a major version bump and a migration note in this file under a `## Migrating from vX to vY` section.
