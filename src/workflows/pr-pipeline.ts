...
export function buildPrPipeline(config: PrPipelineConfig): Workflow {
  const {
    ...
    awsRegion = "us-east-1",
    cdkStackName,
    gpCliRepo = "${{ github.server_url }}/${{ github.repository }}",
  } = config;

  const stackName = cdkStackName ?? `${service.replace(/-/g, "")}Stack-sandbox`;
  ...
  const validateConventions: Job = {
    ...
    steps: [
      ...
      setupPython(pythonVersion),
      {
        ...
        run: [
          "pip install uv",
          `uv tool install "${gpCliRepo}#subdirectory=devex/cli"`,
        ].join("\n"),
      },
      {
      ...
      {
        name: "Enforce two-reviewer rule",
        ...
        with: {
          script: [
            ...
            `});`,
            `const required = ${config.workIdPrefix ? 2 : 2};`,
            `const reviewers = pr.data.requested_reviewers.length + pr.data.requested_teams.length;`,
            `if (reviewers < required) {`,
            `  core.setFailed(\`PR requires at least \${required} reviewers, got \${reviewers}\`);`,
            `}`,
          ].join("\n"),
        },
      },
